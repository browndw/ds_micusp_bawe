...
Our study compares disclinary variation in student writing across two national contexts: American and British. The American data come from the Michigan Corpus of Upper-Level Student Papers (MICUSP), while the British come from the British Academic Written English (BAWE) corpus.
...
```{r}
ds_micusp[1:10, 1:6] %>% knitr::kable(digits = 3)
```
...
```{r echo=FALSE}
 micusp_counts %>% janitor::adorn_totals() %>% knitr::kable("html") %>% kableExtra::row_spec(18, bold = TRUE) %>%
...
```{r echo=FALSE}
 bawe_counts %>% janitor::adorn_totals() %>% knitr::kable("html") %>% kableExtra::row_spec(32, bold = TRUE) %>%
  kableExtra::kable_styling()
```
...
```{r}
plot_radar(mda_micusp, mda_bawe, factor_n = 1)
```
